...
- There is no default-ref setting. `install` uses the remote default branch unless you pass `--ref` or the `owner/repo@ref` form. `update` also uses the remote default branch when no ref is supplied, clearing any recorded pin; pass a ref again when you want an update to remain pinned.
